...
await pg.evaluate((pp) => { const hero = document.querySelector(".hero"); const end = hero.offsetTop + hero.offsetHeight - innerHeight; scrollTo(0, hero.offsetTop + (end - hero.offsetTop) * pp); }, Number(process.env.P ?? 0.6));
await new Promise((r) => setTimeout(r, 1500));
console.log(await pg.evaluate(() => ({ morph: window.__bdMorph, u: window.__bdMorphU, p: window.__bdProgress, screen: window.__bdScreen })));
